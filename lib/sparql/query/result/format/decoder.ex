defmodule SPARQL.Query.Result.Format.Decoder do
  ...
  @doc """
  Decodes a serialized `SPARQL.Query.ResultSet` from the given string.

  It returns an `{:ok, result_set}` tuple or `{:error, reason}` if an error occurs.
  """
  @callback decode(String.t, keyword) :: {:ok, SPARQL.Query.ResultSet.t} |
                                         {:error, any}
  @doc """
  Decodes a serialized `SPARQL.Query.ResultSet` from the given string.
  ...
  """
  @callback decode!(String.t, keyword) :: SPARQL.Query.ResultSet.t
  ...
  defmacro __using__(_) do
    quote bind_quoted: [], unquote: true do
      ...
      alias SPARQL.Query.{Result, ResultSet}
